...
= { { 0.00 },{ 0.33 },{ 0.70 },{ 0.80 },{ 0.99 },{ 1.00 },{ 0.96 },{ 0.87 },{ 0.60 },{ 0.4 },{ 0.05 } };
...
int main()
{
	...
	for (int epoch = 0; epoch <= MAX_EPOCH; epoch++)
	{
		...
		if (epoch == 0 || epoch == 10000 || epoch == 50000 || epoch == 100000 || epoch == 500000 || epoch == 1000000 || epoch == 2000000 || epoch == 5000000 || epoch == 10000000 || epoch == 15000000)
		{
			for (p = 0; p < NUM_TEST_DATA; p++)
			{
				Forward(test_point[p], weight_kj, weight_ji, bias_k, bias_j, hidden, output);
				for (k = 0; k < NUM_OUTPUT; k++)
				{
					error2 += (output[k] - (4 * test_point[p][0])*(1 - test_point[p][0]))*(output[k] - (4 * test_point[p][0])*(1 - test_point[p][0])) / 2;
				}
			}
			printf("epoch = %d\ntraining error = %f\t testing error = %f\n", epoch, error, error2);
		}
